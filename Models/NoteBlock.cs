...
public class NoteBlock : INotifyPropertyChanged
{
    ...
    // Write-only snapshot for background JSON serialization (see TodoStore.SaveAsync) - never
    // exposed to any bound UI, so a shallow copy of ChecklistItems (sharing the same item
    // instances rather than cloning each one) is enough to stop a background writer's enumerator
    // from racing a concurrent Add/Remove on the live collection.
