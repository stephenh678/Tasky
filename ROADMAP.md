...
**2026-09-10, Tasky Web/Mobile review batch (commit `535199f`):** a full review of `docs/` (the
...
| 106 | Web App Background Sync API | Web | | P3 | M | Sync Engine | **Resolved** | The need is met without the API (2026-09-10, with #7): edits made offline live in the IndexedDB local copy and are pushed by the `online` listener while the tab is open, or reconciled at the next launch if it wasn't. The Background Sync API would only add "upload while the tab is closed" - and Safari doesn't implement it at all, so it could never be the primary path. Re-open only if closed-tab replay on Android specifically becomes a requirement. |
