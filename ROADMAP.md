...
151 total items. 67 active, 13 deferred/resolved, 71 done (see table — `Status` column).

**2026-09-10, Tasky Web/Mobile review batch — shipped as v1.2.0 (commit `535199f`, release tag `v1.2.0`):** a full review of `docs/` (the
shared Web + Mobile PWA; Desktop read only for parity) produced `review_web_mobile.md` in the repo
root - 4 P0 data-loss paths, 22 P1 bugs/parity breaks, 15 mobile UX items and 8 code-quality notes -
and every one of them was then fixed in six batches, with no change to the shared `.tasky` wire
format. Highlights: Desktop's inline Rtf attachments are no longer destroyed by a web edit; a
background merge can no longer orphan the open editor; failed autosaves retry; Done/Trashed tasks
lock like Desktop's `IsEditable`; bulk Mark Done spawns recurrences; every permanent-delete path
cleans up Drive attachments; silent token refresh only treats a 401 as a dead session; a boot guard
explains an old-browser failure instead of a blank sign-in screen. The big roadmap change is
**gating decision #4 reversed**: Tasky Web now precaches its shell in a service worker and keeps an
IndexedDB local copy, so a killed-and-relaunched PWA on a train opens on the tasks it had and syncs
when Drive is reachable again - #6, #7 and #41 move to Done, #106 to Resolved. Also shipped: place
restore with `#task=<id>` deep links, a Today-first tab bar, relative due labels with a time pill,
photo lightbox/camera/pre-upload downscale, share-sheet exports and a per-task Share, a header sync
indicator, long-press multi-select, checklist Enter/Backspace/Alt+arrows, auto-discard of abandoned
empty new tasks, persisted sort, dialog focus traps and valid row ARIA, reduced-motion support and
iPad install-hint detection. Code quality: shared `dialog.js` (`openDialog`/`trapFocus`),
`storage.js`, `snapshot.js`, `boot-guard.js` extracted from `app.js` (progress on #136, not
the split itself); `check-dark-palette.js` added to CI next to `check-cache-version.js`; localhost
CORS on the Cloud Functions now opt-in via `ALLOW_LOCAL_DEV_ORIGIN=true`. 22 new parity tests
(99/99), cache-bust v22 → v23. What could not be verified in this environment is listed as the new
#151 - a short on-device pass. **Released as v1.2.0** (minor bump rather than 1.1.10: this is a
feature release, not a patch) - version synced via `sync-desktop-version.js` (cache-bust v23 → v24),
tagged, and published as a GitHub Release with a freshly built desktop zip even though the desktop
code is unchanged, so `Help → Check for Updates` and the version shown on Tasky Web's sign-in screen
stay in step rather than pointing at a desktop build that doesn't exist.

**2026-09-02, second quick-wins batch:** three more small, self-contained Desktop items picked
after #118 was deferred - #64 (attachment-reference scan of the local `.tasky` file cached by
`LastWriteTimeUtc` instead of re-parsed twice per sync and again on every later sync pass), #65
(decoded `BitmapImage`s cached by path in `NoteBlockToPhotoSourceConverter`, capped at 200 with
oldest-first eviction, instead of re-decoded on every list/editor re-render), and #123 (extracted
a shared `FindBlockElementAtPoint` hit-test helper used by 3 of `MainWindow.xaml.cs`'s 4
near-identical `BlockUIContainer` loops, and merged the byte-identical `Body_`/`NoteBody_`
DragEnter/DragOver handler pairs into one method each - the row spells out what was deliberately
left alone and why). #134 (drop the WinForms `NotifyIcon` dependency) was picked up in the same
breath but turned out riskier on inspection - needs a new third-party dependency and touches tray
icon behavior this session can't interactively verify - so the user chose to skip it this batch
rather than ship it unverified; re-scoped as its own pass in the row. 2 new tests (254/254 total),
build clean.

**2026-09-02, defer #118:** discussing what's left on the roadmap, user chose to defer #118 (move the
Desktop OAuth secret out of source) rather than re-pick it up soon - the one prior attempt at this
(2026-08-28, PKCE-only) broke real sign-in for every user of the client, a worse regression than the
plaintext-secret-in-source problem it was meant to fix. Marked **Deferred** rather than left Active,
with a note that any future attempt needs to be verified against a real Google token exchange before
it ships, not estimated as a quick pass. No code changes.

**2026-09-02, quick-wins batch + packaging re-scope + Web priority parity:** four small
review-backlog performance/data items picked up together (#125: `ListBackups` now scans a backup
file's bytes with `Utf8JsonReader` for its task count instead of fully deserializing it; #129:
hoisted `TodoStore.SaveAsync`/`SettingsStore.Save`'s per-call `JsonSerializerOptions` to a shared
static; #130: `GoogleDriveService`'s file/attachment downloads now stream straight into a temp
file instead of buffering the whole thing into a `MemoryStream` first, then move it into place
atomically; #131: `TaskItem.Notes`/`Links`/`Photos` no longer serialize into the wire format once
empty - which is to say on nearly every task saved by this app in a long time - via a
`[JsonIgnore]` + null-when-empty shadow-property pattern that still deserializes an old file's
populated values for `TodoStore.MigrateToBody`). 6 new tests total. Also closed out #26-28
(code-signing/installer/auto-update engine) as **Resolved** rather than left dangling Active -
#150's `Check for Updates` already solved the actual underlying problem (SmartScreen warnings,
no update mechanism) differently than these three originally assumed, so they were re-scoped to
"re-open only if X specifically becomes the real requirement" instead of staying on the active
list unchanged. Separately, a Desktop/Web feature-parity audit found one concrete, previously-
documented gap - #30's Priority field had data-model/sync/search support on Web but no UI at all -
and shipped it (new priority pill in the editor, colored list-row dot matching desktop's exact
colors). Web/mobile share one codebase (`docs/`), so this is one fix for both.
...
| 6 | Implement Service Worker (sw.js) for PWA | Web | | P0 | M | Offline PWA | **Done** | Shipped in v1.2.0 (2026-09-10, review item U1, reversing gating decision #4). `docs/sw.js` precaches the whole shell (index.html, manifest, icons, styles, every JS module - each with the same `?v=NN` suffix the page uses, so `bump-cache-version.js` invalidates the cache too), serves navigations network-first with the cached `index.html` as the fallback, versioned assets cache-first, everything else network-first, and deletes older caches on activate. Same-origin GET only; Drive/auth traffic is never cached. Its `SHELL_FILES` list must be extended whenever a module is added (dialog.js/storage.js/snapshot.js/boot-guard.js are in it). |
| 7 | Add IndexedDB offline local storage cache | Web | | P0 | L | Data Engine | **Done** | Shipped in v1.2.0 (2026-09-10, review item U1). `docs/js/snapshot.js` (plain IndexedDB, no idb-keyval - one store, one key) holds a debounced local copy of `appState` plus the Drive file/folder ids, the dirty flag and the account email. Boot: if Drive can't be reached for any reason other than "signed out" / "Drive scope missing", `restoreFromSnapshot` opens on the local copy (status "Showing your local copy…", sync indicator in `local`/`offline` state) and a forced sync merges it once `online` fires or Retry is tapped. A copy left dirty by a killed tab is reconciled against the freshly downloaded file exactly like a two-device sync (`reconcileLocalSnapshot` in sync.js, 2 parity tests). Cleared on sign-out and never adopted for a different account. |
...
| 26 | Code-sign application binary | Desktop | | P1 | M | Packaging | **Resolved** | Superseded by #150 (2026-09-02 review): the actual goal here was "stop scaring people with SmartScreen," and #150's `Check for Updates` reaches that without a paid signing cert or Azure Trusted Signing pipeline. Re-open only if a real installer (#27) or store distribution later makes an unsigned binary a hard blocker rather than a one-time warning. |
| 27 | Ship a real installer (MSIX / WiX / Inno) | Desktop | | P1 | L | Packaging | **Resolved** | Superseded by #150 (2026-09-02 review): the manual zip + `Check for Updates` combination already gives installs and updates without MSIX/WiX/Inno's packaging overhead. Re-open if enterprise deployment (#110) or store listing ever becomes a real goal - both genuinely need a proper installer, unlike day-to-day updates. |
| 28 | Add auto-update engine (Velopack / MSIX) | Desktop | | P1 | L | Packaging | **Resolved** | Superseded by #150 (2026-09-02 review): #150's `UpdateService`/`Help → Check for Updates` already delivers this - not silent/automatic-on-launch the way Velopack would be, but that was a deliberate choice (see #150's row and the Status section) to avoid the SmartScreen/cert problem a background updater would reintroduce. Re-open only if silent background updates specifically (not just "can update") become the actual requirement. |
...
| 30 | Add priority field / flag levels | Both | ⚡ | P1 | M | Core Feature | **Done** | New `TaskPriority` enum (None/Low/Medium/High) on `TaskItem.Priority`, carried through `Clone()`/`ApplyTaskFields`/`sync.js` for sync parity. Desktop: Priority `ComboBox` in the detail panel, colored dot on list rows, `PriorityHighest` sort, "High Priority" quick filter. Covered by `TaskComparerTests`/`SyncMergeTests`; review_tasks.md's Critical/High item, checked off there 2026-08-25 but this row wasn't updated until the 2026-08-25 merge pass. **2026-09-02 parity pass: Web UI shipped**, closing the "no web UI yet" gap - new Priority pill (`#editor-priority`, None/Low/Medium/High `<select>`) in the editor's metadata row between Due and Repeat, plus a colored list-row dot (`.task-priority-dot`, same three colors as desktop's `PriorityColorConverter` - reused verbatim from `Themes/LightTheme.xaml`/`DarkTheme.xaml`'s hex values via a new `--warning` CSS token, `--accent`/`--danger` already existed) mirroring desktop's row Ellipse. The `is:highpriority` search operator and merge parity already existed; only the UI to actually set/see it on Web was missing. Deliberately left out: a "High Priority" quick-filter chip - desktop's own row already calls this "desktop-only" by design (the search operator covers the same filtering need without spending scarce mobile chip-row space), so not reopened here. |
...
| 41 | Offline sync conflict notification banner | Web | | P1 | M | Sync Engine | **Done** | Arrived with #7 in v1.2.0 (2026-09-10): when a dirty local copy is reconciled against the cloud file at boot, `reconcileDirtySnapshot` runs the ordinary three-way merge, so a same-task-both-sides edit produces a "(conflicted copy)" exactly as a two-device sync would, and the existing merge notice reports the added/updated/conflicted counts; the status line reads "Loaded, with unsaved edits recovered from your last session". |
...
| 64 | Stop re-scanning save file on every sync | Desktop | | P2 | M | Performance | **Done** | `GetReferencedAttachmentFilenames` (a full `File.ReadAllText` + `JsonDocument.Parse` + per-block RTF regex scan of the local `.tasky` file) used to be called twice per sync (once for `Attachments`, once for `InlineImages`) and again on every later sync pass with no change in between. New `GetReferencedAttachmentFilenamesCached` (`GoogleDriveService.cs`, `internal` for testability) wraps it with an instance-level cache keyed by the file's own `LastWriteTimeUtc` - a real edit forces a fresh scan, an unchanged file (the common case for a while-idle auto-sync) reuses the last one. Scoped to caching the existing scan rather than incremental per-edit tracking (the row's original phrasing) - a smaller, lower-risk change that removes the same repeat-cost. 2 new tests. |
| 65 | Cache decoded bitmaps in photo converters | Desktop | | P2 | M | Performance | **Done** | `NoteBlockToPhotoSourceConverter.Convert` runs on every list/editor re-render and used to decode a brand-new `BitmapImage` from disk every time, even when the file hadn't changed. Now caches the frozen `BitmapImage` by path + `LastWriteTimeUtc` (safe to share since `Freeze()` already made it immutable), capped at 200 entries with oldest-first eviction - same shape as Tasky Web's `#70` thumbnail cache. Not unit-tested (no prior converter test coverage exists in this codebase to extend, and `BitmapImage` decoding is UI-thread/WPF-imaging-pipeline behavior a plain xunit test can't meaningfully exercise) - verified via build + code review only. |
...
| 106 | Web App Background Sync API | Web | | P3 | M | Sync Engine | **Resolved** | The need is met without the API (v1.2.0, with #7): edits made offline live in the IndexedDB local copy and are pushed by the `online` listener while the tab is open, or reconciled at the next launch if it wasn't. The Background Sync API would only add "upload while the tab is closed" - and Safari doesn't implement it at all, so it could never be the primary path. Re-open only if closed-tab replay on Android specifically becomes a requirement. |
...
| 118 | Move Desktop OAuth secret fully out of source (loopback PKCE) | Desktop | | P1 | S | Security | **Deferred** | **Tried and reverted, 2026-08-28**: removed `DefaultClientSecret` and sent an empty secret for the default client, on the theory that Desktop/installed-app OAuth clients rely on PKCE alone (RFC 8252). A live sign-in test came back `invalid_request: client_secret is missing` straight from Google's token endpoint - this specific registered client's type does require it, so the PKCE-only approach doesn't work as-is and the secret was restored to get sign-in working again. **Deferred 2026-09-02** (user's call): that attempt broke sign-in for every user of the client, which is a worse outcome than the status quo it was meant to fix - re-open only with a design that's been shown not to break sign-in before it ships (e.g. a gitignored local file baked in at release-build time, verified against a real Google token exchange first), not as a quick S-effort pass. |
...
| 123 | De-duplicate MainWindow.xaml.cs hit-testing & drag-drop code | Desktop | | P2 | M | Code Quality | Done | Two of the three described duplications shipped. **(1) Hit-test loops**: `RichTextBox_PreviewMouseRightButtonDown`, `RichTextBox_ContextMenuOpening`, and `RichTextBox_QueryCursor` each ran their own copy of "walk every `BlockUIContainer`, skip invisible/non-matching children, `TransformToAncestor`+bounds-check against the pointer, swallow per-element transform failures" before doing their own, different thing with whatever they found - extracted into one shared `FindBlockElementAtPoint` helper, safe because block layout is a non-overlapping vertical flow (at most one block's bounds can contain a given point). **Deliberately not touched**: `RichTextBox_PreviewMouseLeftButtonDown`'s loop - it checks multiple different sub-elements (delete button vs. card body vs. image) per iteration with different actions for each, not "find one element, then branch," so folding it into the same helper would need a riskier rewrite this pass couldn't verify against a live UI. **(2) `Body_*`/`NoteBody_*` DragEnter/DragOver pairs**: each pair's two handlers were byte-identical - merged into one `Body_DragEnterOrOver`/`NoteBody_DragEnterOrOver` each, XAML updated to point both events at the shared method. **Not done**: the `Body_Drop`/`NoteBody_Drop` image-insert-sequence dedup - the two handlers' guard/exception-handling logic differs in ways (one requires `SelectedTaskDetail`+positions the caret at the drop point with narrowly-typed catches, the other checks `File.Exists` per file with a broad catch) that would need to be either preserved via a more complex shared helper or explicitly reconciled, and unifying WPF drag/drop routing behavior (`Preview*` tunneling vs. bubbling `Drop` on a nested element) isn't something this pass could verify without a live UI - left as follow-up, same "needs its own pass" call as `#15`'s `FileSessionManager` deferral. Build clean, 254/254 tests passing (no behavior change to the loops touched, verified by inspection - block bounds are non-overlapping so "return first match" is equivalent to the original "loop until match found or exhausted"). |
...
| 125 | Speed up TodoStore.ListBackups | Desktop | | P3 | S | Performance | **Done** | New `CountTasksInBackupFile`/`CountArrayElements` in `TodoStore.cs` scan a backup file's bytes with `Utf8JsonReader`, structurally counting the `Tasks` array's elements without ever materializing a `TaskItem` - replaces the old `JsonSerializer.Deserialize<AppState>` per backup file (up to the 500-file safety cap) in `ListBackups`. Malformed backup files still report a count of 0 rather than throwing, matching prior behavior. 2 new tests in `TodoStoreTests.cs` (`ListBackups_ReportsCorrectTaskCountViaLazyScan`, `ListBackups_MalformedBackupFile_ReportsZeroTaskCountInsteadOfThrowing`). |
...
| 129 | Cache JsonSerializerOptions / source-generated serialization | Desktop | | P3 | S | Performance | **Done** | Hoisted the per-call `new JsonSerializerOptions { WriteIndented = true }` in both `TodoStore.SaveAsync` and `SettingsStore.Save` to a shared `private static readonly` field on each class. Scoped to the static-options hoist named in the row title - didn't add a `JsonSerializerContext` source generator (the row's "also cuts first-save latency" bonus), since that's a larger, separately-scoped change (reflection-based polymorphism/converter behavior to re-verify) than this row's "S" estimate covers. |
| 130 | Stream Google Drive downloads to disk | Desktop | | P3 | S | Performance | **Done** | `DownloadFileAsync` and `DownloadMediaDirectoryAsync` (`GoogleDriveService.cs`) now stream `request.DownloadAsync` straight into a `FileStream` instead of a `MemoryStream` + `ToArray()` double-buffer, then `File.Move(..., overwrite: true)` from a `.download.tmp` path into place - keeping the atomic-replace guarantee the old buffer-then-write approach had for free, so a failed/interrupted download can't leave a half-written file behind. |
| 131 | Trim legacy TaskItem fields from the wire format | Desktop | | P3 | S | Data Engine | **Done** | System.Text.Json has no built-in "omit if empty string/collection" condition (only `WhenWritingDefault`/`WhenWritingNull`, which compare against the CLR default - `null` - not `""`/an empty collection, and these three fields are never actually null). Worked around it with the standard shadow-property idiom: `Notes`/`Links`/`Photos` stay as-is (now `[JsonIgnore]`) for all existing code, and new `NotesForSerialization`/`LinksForSerialization`/`PhotosForSerialization` properties (`[JsonPropertyName]` back to the original wire name, `[JsonIgnore(Condition = WhenWritingNull)]`) return `null` when empty and forward reads/writes to the real property otherwise - so an old file's populated legacy fields still deserialize and still feed `TodoStore.MigrateToBody` exactly as before, but a task that's already been migrated (i.e. nearly every task saved by this app in a long time) no longer carries three dead `"Notes":"","Links":[],"Photos":[]` fields into every save/sync payload. 2 new tests in `TodoStoreTests.cs` covering both the omit-when-empty and still-serializes-when-populated cases, on top of the pre-existing legacy-migration round-trip test. |
...
| 134 | Reconsider the WinForms dependency (NotifyIcon) | Desktop | | P3 | S | Tech Debt | Active | `UseWindowsForms` exists solely for `TrayIconService`'s `NotifyIcon`, requiring global-using suppression hacks in the csproj. A WPF-native tray package (e.g. H.NotifyIcon) or shell interop would drop the second UI framework. From review_tasks.md. **Picked up alongside `#64`/`#65`/`#123` (2026-09-02), then deliberately skipped**: unlike those three, the real fix here means adding a new third-party dependency (or hand-rolling `Shell_NotifyIcon` P/Invoke) and rewriting the one piece of UI this session has no way to interactively verify - tray icon click/double-click/balloon/context-menu behavior, which reminders, quick-add, and show/hide all depend on. User's call: not worth the risk profile as part of a "quick, low-risk" batch; re-scope as its own pass with real device verification before picking back up. |
...
| 136 | Split app.js monolith into modules + patch renders instead of full re-renders | Web | | P1 | L | Architecture | Active | Web's twin of #15 (MainViewModel god object): `app.js` is 1,843 lines of module-global state and DOM wiring, and every keystroke in the title field or search calls `renderList()`, rebuilding the entire task list DOM (rows, icons, swipe listeners) from scratch. Split into state/list-view/editor-view/navigation/install-banner modules; debounce or patch-only the affected row. From review_tasks.md. **Progress (v1.2.0, 2026-09-10):** the patch-render half is done (#67's keyed diff), and the web/mobile review's code-quality pass extracted what could be extracted without the split: `dialog.js` (one `openDialog` behind all four hand-built dialogs, plus `trapFocus`), `storage.js` (guarded localStorage/sessionStorage), `snapshot.js` (IndexedDB local copy), `boot-guard.js`, and one `SHORTCUTS` table driving both the keydown dispatcher and the shortcuts modal. `app.js` is now ~4,300 lines. Bulk actions and quick-add were deliberately *not* moved out: both read and write a dozen module-scope bindings (`appState`, `selectedTaskId`, `selectedIds`, `renderList`, `markDirty`, …), so extracting them is this item proper - a state/context object first, then the modules. |
...
| 151 | On-device verification of the 2026-09-10 web/mobile batch | Web | | P1 | S | Testing | Active | Everything in `review_web_mobile.md` (shipped as v1.2.0) was verified by parity tests, syntax checks and a browser harness, but this environment has no phone. A short pass on a real iPhone and a real Android with the live site: (1) long-press a task row starts multi-select without also opening the task (iOS fires a click when the finger lifts; Android fires contextmenu mid-hold - both are handled, neither was exercised); (2) checklist Enter/Backspace caret placement with Gboard, which sends Enter as an `insertLineBreak` input event rather than a keydown; (3) the due date and time pills open the OS picker from a tap anywhere on the pill (the native input is stretched invisibly over it); (4) the share sheet path for exports, file attachments and Share Task (`navigator.canShare({ files })`); (5) kill the installed PWA, go offline, relaunch - it should open on the local copy and sync once back online; (6) the iOS install hint now shows on iPad. Fix anything that misbehaves; nothing else is expected to need changing. |
