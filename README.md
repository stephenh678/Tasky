...
- Quick filters (Overdue, Due Today, No Due Date, Recurring, With Link, With Attachment, High
...
- **Welcome to Tasky** tour on first launch — Quick Add syntax, where to find it, search
  operators, and Google Drive/Tasky Web sync, with an opt-in "Add 4 sample tasks" checkbox.
  Replayable anytime via **Help → About Tasky → Replay welcome tour**

### Staying up to date
- **Help → Check for Updates** looks for a newer release on GitHub and, if found, offers to
  download and install it — no separate installer, browser download, or admin rights needed
- Opt-in silent daily check in the background (**Settings → Automatically check for updates**,
...
- **Quick-add popup** — long-press (or right-click) any **+ New Task**/**Add Task** button for a floating
  ...
  looking at — the web/PWA equivalent of the desktop app's tray-icon quick-add. Installing Tasky
  Web to a home screen or desktop also adds a "New Task" shortcut on its app icon (long-press on
