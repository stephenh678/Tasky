...
public class GoogleDriveService
{
    ...
    // _driveService/_credential with fresh instances - fine the first time, but a re-auth (retry
    // after a transient failure, switching Google accounts, a second "Connect" click) leaked the
    // previous DriveService's HttpClient/handler (it's IDisposable via BaseClientService) every
    // time. Centralized here and called before every reassignment, plus from SignOut, which
    // previously just nulled the fields without disposing either.
    private void DisposeCurrentDriveSession()
    {
    ...
    // ROADMAP #118: an earlier pass tried dropping this and sending an empty secret for the
    // default client, on the theory that Desktop/installed-app OAuth clients rely on PKCE rather
    // than a confidential secret (RFC 8252). A live sign-in test against this specific registered
    // client came back "invalid_request: client_secret is missing" - Google's token endpoint does
    // require it for this client's registration, so the secret-less approach doesn't work and was
    // reverted. Left as-is (Active, not Done) pending a real fix for keeping this out of source.
    public const string DefaultClientSecret = "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";
    ...
    public async Task<bool> AuthenticateAsync(string? clientId = null, string? clientSecret = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            DisposeCurrentDriveSession(); // release a prior session before this one replaces it - see #61's comment above
            ...
            _credential = await GoogleWebAuthorizationBroker.AuthorizeAsync(
                ...
                "user",
                cancellationToken,
                new DpapiFileDataStore(tokenDataPath));
    ...
    public async Task<bool> TrySilentAuthenticateAsync(string? clientId = null, string? clientSecret = null)
    {
        ...
        try
        {
            ...
            var credential = await GoogleWebAuthorizationBroker.AuthorizeAsync(
                ...
                new DpapiFileDataStore(tokenPath));
            ...
            if (credential.Token is null || credential.Token.IsStale)
            {
                ...
                if (!refreshed)
                {
                    (credential as IDisposable)?.Dispose(); // never made it into _credential - don't leak it
                    return false;
                }
            }
    ...
    public async Task<string?> FindExistingFileIdAsync(string fileName, string parentFolderId)
    {
        ...
        var q = $"name = '{EscapeDriveQueryValue(fileName)}' and '{EscapeDriveQueryValue(parentFolderId)}' in parents and trashed = false";
        return await FindFileIdWithIndexLagRetryAsync(q, $"File '{fileName}'");
    }

    // Drive's q parameter syntax escapes a literal single quote or backslash inside a string
    // value with a leading backslash (https://developers.google.com/drive/api/guides/ref-search-terms).
    // ROADMAP.md #58: an unescaped filename/folder name containing either character (a user can
    // type both into a Windows filename via Save As) breaks the query - at best a spurious "not
    // found" that creates a duplicate remote file, at worst a malformed query Drive rejects outright.
    internal static string EscapeDriveQueryValue(string value) => value.Replace("\\", "\\\\").Replace("'", "\\'");
    ...
    public async Task<string> UploadFileAsync(string localPath, string? existingRemoteFileId = null, Settings? settings = null, SettingsStore? settingsStore = null)
    {
        ...
        // ROADMAP #127: fires on every single sync - routine, not a notable event - demoted from
        // Info to Debug.
        AppLogger.Debug("GoogleDriveService", $"Uploading file '{localPath}' to Google Drive");
    ...
    private async Task SyncMediaDirectoryAsync(string dirName, string localDataFilePath, string containerFolderId, Settings? settings = null, SettingsStore? settingsStore = null)
    {
        ...
        try
        {
            var localDir = MediaPathResolver.DirectoryFor(localDataFilePath, dirName);
    ...
    /// </summary>
    // Public rather than private: these three are the pure, non-network parts of this file's
    // 3-way-diff media sync (filename extraction from task JSON, and the "was this file part of
    // the last sync" bookkeeping) - exposed so TodoApp.Tests can cover the actual bug-prone logic
    // (regex/JSON parsing, legacy-key fallback) directly, without needing a fake Drive client for
    // the surrounding methods that do the real network calls.
    public static HashSet<string> ResolveLastSyncedMediaSet(string fileKey, Settings? settings)
    {
    ...
    public static HashSet<string> GetReferencedAttachmentFilenames(string localDataFilePath)
    {
    ...
    public static void ParseBlockReferences(JsonElement block, HashSet<string> set)
    {
        var photoPath = block.TryGetProperty("PhotoPath", out var pProp) && pProp.ValueKind == JsonValueKind.String
            ? pProp.GetString() : null;
        var rtf = block.TryGetProperty("Rtf", out var rtfProp) && rtfProp.ValueKind == JsonValueKind.String
            ? rtfProp.GetString() : null;

        TaskMediaHelper.CollectReferencedFileNames(photoPath, rtf, set);
    }
    ...
    public async Task DownloadFileAsync(string remoteFileId, string destinationLocalPath, bool downloadAttachments = true, Settings? settings = null, SettingsStore? settingsStore = null)
    {
        ...
        // ROADMAP #127: fires on every single sync that has an existing remote file - routine, not
        // a notable event - demoted from Info to Debug.
        AppLogger.Debug("GoogleDriveService", $"Downloading remote file ID '{remoteFileId}' to '{destinationLocalPath}'");
