...
public partial class GoogleDriveSettingsControl : UserControl
{
    ...
    private readonly Func<Task<bool>> _onCreateNewLocalFile;
    ...
    public GoogleDriveSettingsControl(
        ...
        Func<string, string, Task<bool>> onAttachExistingRemoteFile, Func<Task<bool>> onCreateNewLocalFile)
    {
        InitializeComponent();
    ...
    private async Task<bool> OfferFilePickerAsync(bool silentIfNoneFound)
    {
        ...
        if (picker.Result == GoogleDriveFilePickerResult.CreateNew)
            return await _onCreateNewLocalFile();
    ...
    private void ImportJson_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                ThemedMessageBox.Show("Could not find 'installed' or 'web' property in the selected JSON file.", "Couldn't Import Credentials", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        catch (Exception ex)
        {
            ThemedMessageBox.Show($"Couldn't read client_secret.json: {ex.Message}", "Couldn't Import Credentials", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
